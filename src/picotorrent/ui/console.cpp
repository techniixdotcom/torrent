...
Console::Console(wxWindow* parent, wxWindowID id, pt::UI::Models::TorrentListModel* model, bool isDarkMode)
    ...
    m_model(model)
{
    m_input->SetFont(nx::Theme::MonoFont(9));
    m_input->SetBackgroundColour(nx::Theme::Panel());
    m_input->SetForegroundColour(nx::Theme::Green());
    ...
    (void)isDarkMode; // NiiX Torrent is dark-only
    this->SetBackgroundColour(nx::Theme::Panel());
