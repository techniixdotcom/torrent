...
bool TorrentListModel::GetAttrByRow(unsigned int row, unsigned int col, wxDataViewItemAttr& attr) const
{
    ...
    // Alternating row background for readability: even rows keep the default
    // (theme) background, odd rows use the NiiX accent (#e75480).
    if (row % 2 == 1)
    {
        attr.SetBackgroundColour(wxColour(231, 84, 128));
        attrSet = true;
    }

    // A torrent with a coloured label overrides the alternating stripe.
    if (torrent->Label() > 0
