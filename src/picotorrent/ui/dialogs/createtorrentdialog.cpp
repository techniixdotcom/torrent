    ...
    m_session(session)
{
    ...
    m_creator = new wxTextCtrl(optionsSizer->GetStaticBox(), wxID_ANY, fmt::format("NiiX Torrent {0}", pt::BuildInfo::version()));
    ...
    buttonsSizer->Add(new wxHyperlinkCtrl(this, wxID_ANY, i18n("how_to_create_torrents"), "https://github.com/niixtorrent/niixtorrent"), 0, wxALIGN_CENTER_VERTICAL);
    ...
    this->Bind(ptEVT_CREATE_TORRENT_THREAD_ERROR,
        [this](wxThreadEvent& evt)
        {
            ...
            wxMessageBox(err, "NiiX Torrent", wxICON_ERROR, this);
...
void CreateTorrentDialog::OnCreateTorrent(wxCommandEvent&)
{
    ...
    if (!fs::exists(p))
    {
        wxMessageBox(i18n("no_such_file_or_directory"), "NiiX Torrent", wxICON_WARNING, this);
        m_path->SetFocus();
        return;
    }
