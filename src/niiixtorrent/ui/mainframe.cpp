...
const char* WindowTitle = "NiiX Torrent";
    ...
    m_ipc(std::make_unique<IPC::Server>(this))
{
    ...
    this->SetBackgroundColour(nx::Theme::Background());
...
void MainFrame::AddTorrents(std::vector<lt::add_torrent_params>& params, bool use_commandline_options)
{
    ...
    if (didRemove)
    {
        if (!m_options.silent || !use_commandline_options)
        {
            ...
            wxMessageBox(err, "NiiX Torrent", wxOK, this);
        }
    }
...
{
    wxLaunchDefaultBrowser("https://github.com/niixtorrent/niixtorrent");
}
...
void MainFrame::OnViewPreferences(wxCommandEvent&)
{
    ...
    if (dlg.ShowModal() == wxID_OK)
    {
        if (dlg.WantsRestart())
        {
            ...
            // Shut the IPC (DDE) server down cleanly before relaunching, so the
            // DDEML doesn't surface a cosmetic "failed to disconnect" dialog as
            // this instance exits.
            {
                wxLogNull suppressDdeTeardownErrors;
                m_ipc.reset();
            }
