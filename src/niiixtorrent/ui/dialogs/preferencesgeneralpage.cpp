...
struct AutoRunKey
{
    ...
    void Create()
    {
        ...
        {
            BOOST_LOG_TRIVIAL(warning) << "NiiX Torrent could not be registered to run at start-up. Error: " << GetLastError();
        }
    }
    ...
    m_cfg(cfg)
{
    ...
    m_showNotificationIcon = new wxCheckBox(notifSizer->GetStaticBox(), wxID_ANY, i18n("show_niixtorrent_in_notification_area"));
...
void PreferencesGeneralPage::Save(bool* restartRequired)
{
    ...
    if (langData != nullptr)
    {
        if (langData->GetValue() != Translator::GetInstance().GetLocale())
        {
