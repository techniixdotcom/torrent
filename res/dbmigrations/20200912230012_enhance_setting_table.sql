...
UPDATE setting SET value = '"' || (SELECT string_value FROM setting WHERE key = 'update_checks.url') || '"', default_value = '"https://api.niixtorrent.example/releases/latest"' WHERE key = 'update_checks.url';
